document.addEventListener('DOMContentLoaded', function() {
    ...
    let s2Curriculum = {}; // Will be fetched from CSV
    ...
    const populateS2Majors = () => {
        const majors = Object.keys(s2Curriculum);
        s2MajorSelect.innerHTML = majors.map(major => {
            const formattedName = `S2 ${major.charAt(0).toUpperCase() + major.slice(1).replace(/_/g, ' ')}`;
            return `<option value="${major}">${formattedName}</option>`
        }).join('');
    ...
    const populateS2Concentrations = () => {
        ...
        if (!major || !s2Curriculum[major]) return;
    ...
    const populateS2Semesters = () => {
        ...
        if (!major || !concentration || !s2Curriculum[major][concentration]) return;
